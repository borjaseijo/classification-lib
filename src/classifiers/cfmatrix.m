...
% 4. per = 0/1/2 (default = 0)
% This parameter when set to 1 provides the values in the confusion matrix 
% as percentages (with respect to the total number of samples).
% When set to 2 provides the values as percentages (with respect to the
% number of samples per class). The default provides the values in numbers.
%
% Example:
...
elseif (per == 2)
    samplesPerClass = zeros(1,n_class);
    for k = 1:n_class
        samplesPerClass(k) = sum(actual == classlist(k));
    end
    
    for cl = 1:n_class
        confmatrix(:,cl) = (confmatrix(:,cl) ./ samplesPerClass(cl)).*100;
    end
end
